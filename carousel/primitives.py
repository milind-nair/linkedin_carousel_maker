...
def card_block(ctx: "DrawContext", y_top: float, accent_color: Color,
               title: str, body: str, card_bg: Color | None = None) -> float:
    ...
    body_lines = wrap(body, cfg.fonts.body, 11, cfg.content_width - 40)
    card_h = 32 + len(body_lines) * 16 + 16
    ...
    # Title
    c.setFont(cfg.fonts.bold, 12.5)
    c.setFillColor(cfg.colors.primary_dark)
    c.drawString(cfg.margin + 20, y_top - 22, title)
    # Body
...
def insight_block(ctx: "DrawContext", y_top: float, accent_color: Color,
                  ...
                  label_color: Color | None = None, bg: Color | None = None) -> float:
    ...
    body_lines = wrap(body_text, cfg.fonts.body, 10.5, cfg.content_width - 44)
    block_h = 28 + len(body_lines) * 15 + 14
    ...
    c.drawString(cfg.margin + 18, y_top - 20, label)
...
def bottom_takeaway(ctx: "DrawContext", accent_color: Color,
    ...
    """Anchored lower callout so the viewport ends with useful content."""
    cfg = ctx.config
    c = ctx.canvas
    if bg is None:
        bg = HexColor("#FFFEF7")
    ...
    c.setFillColor(accent_color)
    c.drawString(cfg.margin + 16, y_bottom + 37, label.upper())
    c.setFont(cfg.fonts.body, 10)
    c.setFillColor(cfg.colors.text)
    text_x = cfg.margin + 16
    text_y = y_bottom + 20
    for line in wrap(body_text, cfg.fonts.body, 10, cfg.content_width - 32):
